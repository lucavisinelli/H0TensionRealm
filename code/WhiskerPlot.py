...
elp = ErrorLinePloter({'ml':0.0,'e1_sig':[0.1,-0.1]},position=nr+2)
labels[elp.position]=''#Reference, ${H_0}$ [km$\,$s$^{-1}\,$Mpc$^{-1}$]'
elp.plot()

for i in range(len(paras)):
    if method[i]=='Indirect':
      elp = ErrorLinePloter(all_data[i],position=nr-i+1)
    elif method[i]=='Direct':
      elp = ErrorLinePloter(all_data[i],position=nr-i)
...
#plt.text(77.5,66.5,"Indirect",size=9)
#plt.hlines(66, 50, 90, colors = 'black', linestyle = '--', linewidth = 0.5, alpha = 1)
#plt.text(77.5,64.15,"Direct",size=9)
